    ...
    fetch('/.netlify/functions/merkletree?address=' + account)
      ...
      .then(
        (result) => {
          ...
          console.log("alCountPreMint: ", result.alCountPreMint)
      ...
      <>
        ...
        {!data.publicSaleWithoutProof ? //2nd,3rdセールではない時だけ、AL対象かどうかを表示
           data.loading ? 'Loading Your Status...' :
             (alCountPreMint > 0 ? 
                (data.alreadyMinted == 0 ? "AL対象のあなたは " + alCountPreMint + " 点ミントできます" : "1点ミント済みです。ありがとうございます！")
                : "AL対象外です・・。2nd saleでお待ちしてます！")
          : "" }<br />
  ...
  return (
    <ResponsiveWrapper style={{ padding: 24 }} test>
      ...
      >
        ...
        >
          4/6(Thu), 20:00 (Japan time) <br></br>until 4/13(Thu), 21:00<br />
          Mint Amount: 1
        </s.TextDescription>
        <s.SpacerSmall />
        <s.TextTitle
